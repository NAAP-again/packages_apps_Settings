...
import com.nasp.settings.preferences.SystemSettingSeekBarPreference;
...
public class BatteryStyleSettings extends SettingsPreferenceFragment implements
    ...
    private static final String KEY_QS_SHOW_BATTERY_PERCENT = "qs_show_battery_percent";
    private static final String KEY_QS_BATTERY_STYLE = "qs_battery_style";
    ...
    private SystemSettingListPreference mQsBatteryPercent;
    private SystemSettingListPreference mQsBatteryStyle;

    @Override
    public void onCreate(Bundle icicle) {
        super.onCreate(icicle);

        addPreferencesFromResource(R.xml.naap_settings_battery_style);
        ...
        int qsbatterystyle = Settings.System.getIntForUser(getContentResolver(),
                Settings.System.QS_BATTERY_STYLE, BATTERY_STYLE_PORTRAIT, UserHandle.USER_CURRENT);
        int qsbatterypercent = Settings.System.getIntForUser(getContentResolver(),
                Settings.System.QS_SHOW_BATTERY_PERCENT, 0, UserHandle.USER_CURRENT);

        mQsBatteryStyle = (SystemSettingListPreference) findPreference(KEY_QS_BATTERY_STYLE);
        mQsBatteryStyle.setOnPreferenceChangeListener(this);

        mQsBatteryPercent = (SystemSettingListPreference) findPreference(KEY_QS_SHOW_BATTERY_PERCENT);
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (preference == mBatteryStyle) {
            ...
            return true;
        } else if (preference == mQsBatteryStyle) {
            int value = Integer.parseInt((String) newValue);
            int batterystyle = Settings.System.getIntForUser(getContentResolver(),
                    Settings.System.STATUS_BAR_BATTERY_STYLE, BATTERY_STYLE_PORTRAIT, UserHandle.USER_CURRENT);
            mQsBatteryPercent.setEnabled(
                    value != BATTERY_STYLE_TEXT && value != BATTERY_STYLE_HIDDEN &&
                    !(value < 0 && (batterystyle == BATTERY_STYLE_TEXT || batterystyle == BATTERY_STYLE_HIDDEN)));
            return true;
        }
        return false;
    }
